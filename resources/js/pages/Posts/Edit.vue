        ...
        <div class="p-6 max-w-3xl  space-y-4">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div v-if="post.image" class="mb-4">
                    <img v-if="post.image" :src="post.image" alt="Current Image" class="max-w-xs rounded" />
                </div>
                ...
                <div>
                    ...
                    <div v-if="form.errors.scheduled_time" class="text-red-600 text-sm">{{ form.errors.scheduled_time }}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <ul
                        ...
                        <li v-for="platform in platforms.data" :key="platform.id"
                            class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r last:border-r-0 dark:border-gray-600">
                            <div class="flex flex-col items-center">
                                <label :for="'platform-checkbox-' + platform.id"
                                    class="justify-center items-center flex w-full py-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                                    <i v-html="platform.icon"
                                        class="mr-2 px-5 rounded-full border flex items-center h-15 w-15 justify-center text-center"></i>
                                    {{ platform.name }}
                                </label>
                                <input :id="'platform-checkbox-' + platform.id" type="checkbox" :value="platform.id"
